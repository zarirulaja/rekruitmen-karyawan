...
<div class="bg-white rounded-lg shadow-sm p-6">
    <form action="{{ route('hrd.lowongan.update', $lowongan->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="gaji_min" id="gaji_min" value="{{ old('gaji_min', $lowongan->gaji_min) }}"
                    class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">
                @error('gaji_min')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="gaji_max" id="gaji_max" value="{{ old('gaji_max', $lowongan->gaji_max) }}"
                    class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">
                @error('gaji_max')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="status" id="status" required
                    class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">
                    <option value="1" {{ old('status', $lowongan->status) ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ old('status', $lowongan->status) ? '' : 'selected' }}>Draft</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div>
            ...
            <textarea name="deskripsi" id="deskripsi" rows="4" required
                class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">{{ old('deskripsi', $lowongan->deskripsi) }}</textarea>
            @error('deskripsi')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea name="persyaratan" id="persyaratan" rows="4" required
                class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">{{ old('persyaratan', $lowongan->persyaratan) }}</textarea>
            @error('persyaratan')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea name="tanggung_jawab" id="tanggung_jawab" rows="4" required
                class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">{{ old('tanggung_jawab', $lowongan->tanggung_jawab) }}</textarea>
